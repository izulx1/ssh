...
apt install -y python speedtest-cli jq iptables iptables-persistent netfilter-persistent net-tools socat cron dropbear squid neofetch
...
if [[ $(cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g') == "ubuntu" ]]; then
    echo "Setup nginx For OS Is $(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/PRETTY_NAME//g')"
...
systemctl enable ws haproxy udp
systemctl start client server haproxy
systemctl start netfilter-persistent
systemctl restart client server
systemctl restart ws cron dropbear haproxy rc-local sshd nginx
